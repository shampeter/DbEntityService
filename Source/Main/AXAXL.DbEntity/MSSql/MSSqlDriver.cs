	...
	internal class MSSqlDriver : IDatabaseDriver
	{
			...
			) where T : class, new()
		{
			...
			return (IEnumerable<T>)delegateHandle.DynamicInvoke(
				...
				-1,
				new ValueTuple<NodeProperty, bool>[0],
		...
		{
			return this.SelectImplementation<T>(connectionString, node, parameters, whereClauses, orClausesGroup, childInnerJoinWhereClauses, childInnerJoinOrClausesGroup, maxNumOfRow, orderBy, timeoutDurationInSeconds);
		}
			...
			) where T : class, new()
		{
			...
			var primaryWhereStatement = string.Empty;
			if (parameters != null)
			{
				...
				primaryWhereStatement = this.sqlGenerator.CreateWhereClause(node, primaryWhereColumns);
			}
			...
			var (additionalWhereClauses, sqlParameterList) = this.CompileConditions<T>(node, whereClauses, orClausesGroup, tablePrefix, innerJoinMap);
			var additionalWhereStatement = additionalWhereClauses.Count() > 0 ? string.Join(@" AND ", additionalWhereClauses) : string.Empty;
			var innerJoinStatement = this.ComputeInnerJoins(innerJoinMap, tablePrefix);

			var primaryWhereIsBlank = string.IsNullOrEmpty(primaryWhereStatement);
			var additionalWhereIsBlank = string.IsNullOrEmpty(additionalWhereStatement);
			var whereStatements =
						!primaryWhereIsBlank || !additionalWhereIsBlank ?
						@" WHERE " + primaryWhereStatement + (!primaryWhereIsBlank && !additionalWhereIsBlank ? @" AND " : string.Empty) + additionalWhereStatement :
						string.Empty;

			var sqlCmd = string.Format("{0}{1}{2}", select.SelectClause, innerJoinStatement, whereStatements);
